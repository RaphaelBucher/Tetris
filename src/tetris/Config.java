...
public interface Config {
  ...
  int BLOCKS_Y = 18; // Amount of blocks inside the playing field in the
                     // vertical direction
  ...
  int SHAPE_AMOUNT = 7;
  int SHAPEBLOCKS_AMOUNT = 4; // kann NICHT ohne weiteres (ohne massive?
                              // Anpassungengen) verändert werden
  ...
  int SHAPE_AUTOMATIC_SINK_RATE_MILLIS = 900; // The Shape sinks automatically
                                              // every ____ Millis.
  int SHAPE_PLAYER_SINK_RATE_MILLIS = 60; // In case the player presses the
                                          // arrow-down key to let
