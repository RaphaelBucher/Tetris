...
public class Block {
	private Color color;
	private int posX, posY; //starting at 0, 1, ..., the absolute position inside the Board-Matrix
	...
	/**
	 * @param tmpPaintColor paints the blocks temporarily in this color and not in their blocks-color
	 */
	...
	private void paintBlock(Graphics2D graphics2D) {
		graphics2D.fillRect(posX * Config.BLOCK_SIZE + Config.BLOCK_DRAWING_INSETS,
				posY * Config.BLOCK_SIZE + Config.BLOCK_DRAWING_INSETS, 
	...
	private boolean validPosY(int posY) {
		return posY >= -2 && posY < Config.BLOCKS_Y + 2; // -2 ist erlaubt (instant rota bei neuem shape) 
	}
	...
	/**
	 * Own implementation, returns a new Block-object and not a reference on the existing one.
	 */
