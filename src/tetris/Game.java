...
public class Game extends JPanel {
  ...
  public Game() {
    this.setPreferredSize(new Dimension(Config.WINDOW_X, Config.WINDOW_Y)); // ***x***
                                                                            // drawable
  ...
  public void paint(Graphics graphics) {
    ...
    // Meine Game-Objekte sollen nicht hier gezeichnet werden, sondern in den
    // jeweiligen
    // Klassen. Bsp:
    ...
    if (gameOver)
      ...
      board.paint(graphics2D, blinks); // paint full rows in normal block-color
                                       // (if there are any??)
    }
  ...
  public void init() {
    board = new Board(); // muss VOR der Shape instantiierung sein... (Shape
                         // holt sich die board-Referenz hier)...
  ...
  /**
   * The main-loop. Manages fps itself via Thread.sleep(). Care: Ist NICHT die
   * run-Methode eines Threads, sondern eine eigene Methode.
   */
  public void run() {
    long startTime;
  ...
  private void update() {
    ...
    if (!shape.sink(tastaturAdapter.isVkDownPressed())) {
      // Shape collided in his sinking-attempt => new shape needed, old shape
      // materializes into the board
  ...
  private void letFullRowsBlink() {
    ...
    while (System.currentTimeMillis() - startTime < Config.FULL_ROW_BLINKING_TIME) {
      ...
      long timeout = Config.FULL_ROW_BLINKING_INTERVALL - (System.currentTimeMillis() - blinkStart);
      sleep(timeout);
    }
