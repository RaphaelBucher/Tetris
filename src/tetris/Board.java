...
public class Board {
	...
	/**
	 * @param fullRowsBlinkColor pass null for normal block painting, pass a Color if you
	 * want to paint full rows in a special color
	 */
	...
	private boolean isBlockWithinBoardArray(Block block) {
		return block.getPosY() >= 0 && block.getPosY() < Config.BLOCKS_Y &&
				block.getPosX() >= 0 && block.getPosX() < Config.BLOCKS_X;
	}
	
	/**
	 * Checks for all kinds of collisions (borders as well as the occupied Board-Blocks).
	 * @return true if a collision occured.
	 */
	public boolean checkShapeCollision(Shape shape) {
		boolean collides = collidesWithBorders(shape) ||
				collidesWithExistingBlocks(shape);
	...
	private boolean collidesWithBorders(Block block) {
		return block.getPosX() < 0 || block.getPosX() >= Config.BLOCKS_X ||
				block.getPosY() >= Config.BLOCKS_Y;
	}
	...
	private boolean isRowFull(int row) {
		for (int j = 0; j < Config.BLOCKS_X; j++) { //iterate over all collumns of a certain row
	...
	public void deleteFullRows() {
		...
		for (int i = Config.BLOCKS_Y - 1; i >= 0 + fullRows.size(); i--) {
			while (grabBlocksOffset < fullRows.size() && 
					fullRows.get(grabBlocksOffset) == i - grabBlocksOffset) {
				grabBlocksOffset++;
			}
	...
	private void copyRow(int copyIntoRow, int grabBlocksOffset) {
		for (int j = 0; j < Config.BLOCKS_X; j++) {
			if (blocks[copyIntoRow - grabBlocksOffset][j] == null)
				...
				blocks[copyIntoRow][j] =
				new Block(blocks[copyIntoRow - grabBlocksOffset][j].getColor(), j, copyIntoRow);
		}
	}
