...
public abstract class Shape {
  ...
  private RelativeBlock[] savedStateRelativeBlocks; // for rollbacks in case of
                                                    // collisions
  ...
  public Shape(Color blockColor, int[][] relBlocksRelPos, Board boardReferenceCopy) {
    ...
    relativeBlocks = new RelativeBlock[Config.SHAPEBLOCKS_AMOUNT - 1]; // -1
                                                                       // CenterBlock
  ...
  /**
   * Shapes 3, 4 and 7 will use this default-implementation ("normal" rotation
   * as expected). newX = -oldY, newY = oldX Shapes 1 and 2 use their
   * overwritten version.
   * 
  ...
  /**
   * The automatic sinking of the shape. This method is called once every
   * Game.update() = once every frame.
   * 
   * @return returns false only if the shape wanted to move but could not due to
   *         a collision, true otherwise (shape could move or didn't have to
   *         move at all (not yet "movetime" by the timer).
   */
