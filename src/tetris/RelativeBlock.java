...
/**
 * A Shape has 3 relative Blocks which rotate around the Center-Block when the user
 * attempts to rotate the shape.
 */
public class RelativeBlock extends Block {
	private int relativePosX, relativePosY; //in relation to the Center-Block of the shape
	private Block centerBlock; //relative to this block
	
	/**
	 * Will also compute the actual absolutePositions.
	 */
	public RelativeBlock(Color color, int relativePosX, int relativePosY,
			Block centerBlock) {
	...
	private boolean validRelativePos(int relativePosX, int relativePosY) {
		return relativePosX >= -2 && relativePosX <= 2 &&
				relativePosY >= -2 && relativePosY <= 2;
	}
	
	protected boolean invariantOfRelativeBlock() {
		return validRelativePos(relativePosX, relativePosY) && invariantOfBlock() &&
				centerBlock != null;
	}
	
	/**
	 * Own implementation, returns a new RelativeBlock-object and not a reference on the existing one.
	 */
